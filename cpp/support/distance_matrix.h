...
// Test-only; lives in cpp/support/. Main package uses UtilFunctions::jaccard in mcmc_utils.h.
        ...
        moire_parallel::parallel_for_2d(0, num_samples_, 0, num_samples_,
            [&](size_t i, size_t j) {
                if (i == j) {
                    ...
                    const T distance = distance_function_(data[i], data[j]);
                    distance_matrix_.at({i, j}) = distance;
                    distance_matrix_.at({j, i}) = distance;
                }
            });
    }
