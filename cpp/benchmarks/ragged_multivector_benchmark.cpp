...
void benchmark_ragged_element_assignment(const std::array<size_t, 2>& dimensions, const std::vector<size_t>& ragged_dimensions) {
    ...
    for (size_t i = 0; i < dimensions[0]; ++i) {
        for (size_t j = 0; j < dimensions[1]; ++j) {
            for (size_t l = 0; l < ragged_dimensions[j]; ++l) {
                mv.at({i, j, l}) = i;
            }
        }
    }
...
void benchmark_ragged_iteration(const std::array<size_t, 2>& dimensions, const std::vector<size_t>& ragged_dimensions) {
    ...
    long long target = std::accumulate(ragged_dimensions.begin(), ragged_dimensions.end(), 0LL) * dimensions[0];
...
void benchmark_ragged_iterator_iteration(const std::array<size_t, 2>& dimensions, const std::vector<size_t>& ragged_dimensions) {
    ...
    for (size_t i = 0; i < dimensions[0]; ++i) {
        for (size_t j = 0; j < dimensions[1]; ++j) {
            auto [begin, end] = mv.inner_iterators({i, j});
...
int main(int argc, char* argv[]) {
    ...
    // Second dimension must equal number of ragged slots per RaggedMultiVector contract
    dimensions[1] = ragged_dimensions.size();

    moire_bench::BenchmarkSuite suite("RaggedMultiVector benchmarks");
    suite.add("Ragged construction", [&]{ benchmark_ragged_construction(dimensions, ragged_dimensions); });
    suite.add("Ragged element assignment", [&]{ benchmark_ragged_element_assignment(dimensions, ragged_dimensions); });
    suite.add("Ragged iteration", [&]{ benchmark_ragged_iteration(dimensions, ragged_dimensions); });
    suite.add("Ragged iterator iteration", [&]{ benchmark_ragged_iterator_iteration(dimensions, ragged_dimensions); });
    suite.add("Naive ragged construction", [&]{ naive_benchmark_ragged_construction(dimensions, ragged_dimensions); });
    suite.add("Naive ragged element assignment", [&]{ naive_benchmark_ragged_element_assignment(dimensions, ragged_dimensions); });
    suite.add("Naive ragged iteration", [&]{ naive_benchmark_ragged_iteration(dimensions, ragged_dimensions); });
    suite.run_all();
    return 0;
} 