...
Now that we have our data, let's go ahead and run the MCMC. Here we run 80 parallel-tempered chains with a temperature gradient ranging from $[0, 1]$. Thread count defaults to physical cores minus one; override with `num_threads` if needed. We also place a weak prior on relatedness that favors low relatedness.
...
initial_freqs <- prepare_initial_allele_frequencies(
  data = simulated_data,
  population_assignments = rep(1, length(simulated_data$sample_ids)),
  num_populations = 1,
  pseudocount = 1
)
