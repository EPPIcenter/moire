...
mean_moi <- 4
num_biological_samples <- 50
epsilon_pos <- .01
epsilon_neg <- .03

# Generate the number of alleles at each locus
allele_counts <- c(rep(5, 5), rep(10, 5), rep(15, 5), rep(25, 5))
...
res <- moire::run_mcmc(
  ...
  eps_pos_alpha = 10, eps_pos_beta = 990, complexity_limit = 2^12, importance_sampling_depth = 1000,
...
```{r plot_allele_freq_concordance_log}
ggplot(allele_freq_data) +
  geom_errorbar(aes(
    y = log(post_allele_freqs_mean),
    x = log(true_allele_frequency),
    # ymax = post_allele_freqs_mean,
    # ymin = post_allele_freqs_mean)) +
    ymax = log(post_allele_freqs_upper),
    ymin = log(post_allele_freqs_lower))) +
  geom_point(aes(y = log(post_allele_freqs_mean), x = log(true_allele_frequency))) +
  geom_point(aes(y = log(naive_allele_frequency), x = log(true_allele_frequency)), color = "red", alpha = .3) +
  geom_abline(slope = 1, intercept = 0) +
  ylab("Mean Posterior Freqeuncy") +
  xlab("True Frequency") +
  theme_classic(base_size = 12) +
...
mean(abs(allele_freq_data$post_allele_freqs_med - allele_freq_data$true_allele_frequency))
mean(abs(allele_freq_data$naive_allele_frequency - allele_freq_data$true_allele_frequency))
```
