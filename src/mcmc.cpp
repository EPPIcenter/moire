    ...
    for (size_t i = 0; i < params.num_chains; i++)
    {
        ...
        for (int j = 0; j < params.burnin; j++)
        {
            ...
            if ((j + 1) % 10 == 0)
            {
                ...
                UtilFunctions::print_vector(chains[i].m_accept);
                // UtilFunctions::print("Update Eps Pos:");
                // UtilFunctions::print_vector(chains[i].eps_pos_accept);
                // UtilFunctions::print("Update Eps Neg:");
                // UtilFunctions::print_vector(chains[i].eps_neg_accept);
                // UtilFunctions::print("Update Individuals:");
                // UtilFunctions::print_vector(chains[i].individual_accept);
            }
            ...
            // chains[i].update_individual_parameters(params.burnin + j + 1);
...
MCMC::MCMC(GenotypingData genotyping_data, Lookup lookup, Parameters params) : genotyping_data(genotyping_data),
                                                                               lookup(lookup),
                                                                               params(params)
{
