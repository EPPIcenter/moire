...
//     total_locus_alleles(genotyping_data.num_loci); std::vector<int>
//     total_alleles(genotyping_data.num_loci);
...
//                        genotyping_data.num_alleles[i]);  // Make sure at
//                        least 1
//                                                          // allele everywhere
//         }
//     }
...
//             double logitProp =
//                 sampler.sample_epsilon(logitCurr,
//                 params.allele_freq_vars[j]);
...
//             double ls = propLogPQ.second -
//             UtilFunctions::logitSum(logitPropP); logitPropP =
//             UtilFunctions::logitScale(logitPropP, ls);
...
//             // check to make sure the proposed simplex is within a bounded
//             range for (const auto &el : prop_p)
//             {
//                 if (el < 1e-12)
//                 {
//                     return;
...
//                     llik_new[j][i] = calc_genotype_marginal_llik(
//                         observed_alleles, m[i], prop_p, eps_neg[i],
//                         eps_pos[i], true);
...
//             if (k > 12)
//             {
//                 UtilFunctions::print("Sample AF:", sum_can, sum_orig);
...
//             if (params.adapt_allele_freq_vars and iteration < params.burnin
//             and
//                 iteration > 15)  // don't start adapting until there are at
//                                  // least a few samples
//             {
//                 double acceptanceRate =
//                     std::min((p_accept[j] + 1) / (double(iteration) +
//                     1), 1.0);
//                 params.allele_freq_vars[j] +=
//                     (acceptanceRate - .23) / std::pow(iteration + 1, .5);
//                 // (acceptanceRate - .23) / (iteration + 1);
//                 params.allele_freq_vars[j] =
//                     std::max(params.allele_freq_vars[j], 1e-16);
//             }
//             UtilFunctions::print("Accept AF:", p_accept[j] /
//             double(iteration),
//                                  params.allele_freq_vars[j]);
//         }
//     }
...
//                     llik_new[j][i] = calc_genotype_marginal_llik(
//                         genotyping_data.get_observed_alleles(j, i), m[i],
//                         p[j], eps_neg[i], prop_eps_pos, false);
//                     sum_can += llik_new[j][i];
//                     sum_orig += llik_old[j][i];
//                 }
//             }
...
//                     llik_new[j][i] = calc_genotype_marginal_llik(
//                         genotyping_data.get_observed_alleles(j, i), m[i],
//                         p[j], prop_eps_neg, eps_pos[i], false);
//                     sum_can += llik_new[j][i];
//                     sum_orig += llik_old[j][i];
//                 }
//             }
...
// double Chain::calc_observation_process(std::vector<int> const
// &allele_index_vec,
...
//     double epsilon_neg, int coi, std::vector<double> const
//     &allele_frequencies)
// {
//     double res = 0.0;
//     res += calc_transmission_process(allele_index_vec, allele_frequencies,
//     coi);
...
//     // Calculate the distribution over how many false positives there might
//     be.
...
//         // I.e., COI is 5, we observe 6, but sample 2 false positives ->
//         total
...
//         // require at least one allele to be observed. discard samples that
//         do
...
//                 allele_index_vec, obs_genotype, coi, epsilon_neg,
//                 epsilon_pos));
//             // UtilFunctions::print("Prob:", true_prob, obs_prob,
//             // importance_prob, total_false_positives,
//             total_false_negatives); double weight = true_prob /
//             importance_prob; if (!std::isnan(weight) and
//             !std::isnan(obs_prob))
//             {
...
//     std::map<std::vector<int>, std::tuple<double, double, double>>
//     pqf_memo{};
...
//     while (--i >= 0)
//     {
//         allele_index_vec = sampler.sample_latent_genotype(coi,
//         importance_dist); auto pqf_iter = pqf_memo.find(allele_index_vec); if
//         (pqf_iter != pqf_memo.end())
//         {
...
//                 allele_index_vec, obs_genotype, coi, epsilon_neg,
//                 epsilon_pos));
//             pqf_memo[allele_index_vec] = {true_prob, importance_prob,
//             obs_prob}; accumulated_prob += true_prob / importance_prob *
//             obs_prob; accumulated_weight += true_prob / importance_prob;
//         }
//     }
...
//             obs_prob = std::exp(calc_observation_process(
//                 allele_index_vec, obs_genotype, coi, epsilon_pos,
//                 epsilon_neg));
...
//     // double approx1 = calc_estimated_genotype_marginal_llik(
...
//     // UtilFunctions::print("approx1");

//     // double approx2 = calc_estimated_genotype_marginal_llik(
//     //     obs_genotype, coi, allele_frequencies, epsilon_neg, epsilon_pos,
//     //     params.importance_sampling_depth +
//     //         coi * params.importance_sampling_scaling_factor,
//     //     importance_sample);

//     // UtilFunctions::print("approx2");

//     // double approx3 = calc_estimated_genotype_marginal_llik(
//     //     obs_genotype, coi, allele_frequencies, epsilon_neg, epsilon_pos,
//     //     params.importance_sampling_depth +
//     //         coi * params.importance_sampling_scaling_factor,
//     //     importance_sample);

//     // UtilFunctions::print("approx3");

//     // double approx4 = calc_estimated_genotype_marginal_llik(
//     //     obs_genotype, coi, allele_frequencies, epsilon_neg, epsilon_pos,
//     //     params.importance_sampling_depth +
//     //         coi * params.importance_sampling_scaling_factor,
//     //     importance_sample);

//     // UtilFunctions::print("approx4");

//     // double exact = calc_exact_genotype_marginal_llik(
//     //     obs_genotype, coi, allele_frequencies, epsilon_neg, epsilon_pos);
//     // UtilFunctions::print("exact");

//     // UtilFunctions::print("Approx/Exact:", approx1, approx2, approx3,
//     approx4,
//     //                      exact);
...
// Chain::Chain(GenotypingData genotyping_data, Lookup lookup, Parameters
// params)
