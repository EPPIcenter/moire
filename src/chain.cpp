...
void Chain::update_m(int iteration)
{
    for (int i = 0; i < genotyping_data.num_samples; i++)
    {
        ...
        if (params.max_coi >= prop_m && prop_m > 0)
        {
            ...
            {
                llik_new[j][i] = calc_genotype_marginal_llik(genotyping_data.observed_alleles[j][i], prop_m, p[j], eps_neg[i], eps_pos[i]);
            ...
            // if (m[i] < prop_m)
            // {
            //     UtilFunctions::print("Going from ", m[i], "to", prop_m, "with probability", exp(sum_can - sum_orig));
            // }
...
void Chain::update_p(int iteration)
{
    for (size_t j = 0; j < genotyping_data.num_loci; j++)
    {
        ...
        {
            llik_new[j][i] = calc_genotype_marginal_llik(genotyping_data.observed_alleles[j][i], m[i], prop_p, eps_neg[i], eps_pos[i]);
            sum_can += llik_new[j][i];
            sum_orig += llik_old[j][i];
        }
...
void Chain::update_eps(int iteration)
{
    for (size_t i = 0; i < m.size(); i++)
    {
        ...
        if (prop_eps_pos < params.max_eps_pos && prop_eps_pos > 0 && prop_eps_neg < params.max_eps_neg && prop_eps_neg > 0)
        {
            ...
            sum_orig += sampler.get_epsilon_log_prior(eps_neg[i], params.eps_neg_alpha, params.eps_neg_beta);
            sum_orig += sampler.get_epsilon_log_prior(eps_pos[i], params.eps_pos_alpha, params.eps_pos_beta);
...
void Chain::update_eps_pos(int iteration)
{
    for (size_t i = 0; i < m.size(); i++)
    {
        ...
        if (prop_eps_pos < params.max_eps_pos && prop_eps_pos > 0)
        {
            ...
            for (size_t j = 0; j < genotyping_data.num_loci; j++)
            {
                llik_new[j][i] = calc_genotype_marginal_llik(genotyping_data.observed_alleles[j][i], m[i], p[j], eps_neg[i], prop_eps_pos);
                sum_can += llik_new[j][i];
                sum_orig += llik_old[j][i];
            }
            ...
            // UtilFunctions::print("Eps pos:", prop_eps_pos, exp(sum_can - sum_orig));
...
void Chain::update_eps_neg(int iteration)
{
    for (size_t i = 0; i < m.size(); i++)
    {
        ...
        if (prop_eps_neg < params.max_eps_neg && prop_eps_neg > 0)
        {
            ...
            for (size_t j = 0; j < genotyping_data.num_loci; j++)
            {
                llik_new[j][i] = calc_genotype_marginal_llik(genotyping_data.observed_alleles[j][i], m[i], p[j], prop_eps_neg, eps_pos[i]);
                sum_can += llik_new[j][i];
                sum_orig += llik_old[j][i];
            }
            ...
            // UtilFunctions::print("Eps neg:", prop_eps_neg, exp(sum_can - sum_orig));
...
void Chain::update_individual_parameters(int iteration)
{
    for (size_t i = 0; i < m.size(); i++)
    {
        int prop_m = m[i] + sampler.sample_coi_delta(2);
        double prop_eps_neg = sampler.sample_epsilon_neg(eps_neg[i], eps_neg_var);
        double prop_eps_pos = sampler.sample_epsilon_neg(eps_pos[i], eps_pos_var);
            ...
            params.max_coi >= prop_m && prop_m > 0)
        {
            ...
            for (size_t j = 0; j < genotyping_data.num_loci; j++)
            {
                llik_new[j][i] = calc_genotype_marginal_llik(genotyping_data.observed_alleles[j][i], prop_m, p[j], prop_eps_neg, prop_eps_pos);
                sum_can += llik_new[j][i];
                sum_orig += llik_old[j][i];
            }
...
std::vector<double> Chain::calc_genotype_log_pmf(std::vector<std::vector<int>> const &genotypes, int coi, std::vector<double> const &allele_frequencies, int num_genotypes)
{
    ...
    {
        res[i] = gsl_ran_multinomial_lnpdf(allele_frequencies.size(), allele_frequencies.data(), (unsigned int *)genotypes[i].data());
    }
    return res;
}
...
long double Chain::calc_exact_genotype_marginal_llik(std::vector<int> const &obs_genotype, int coi, std::vector<double> const &allele_frequencies, double epsilon_neg, double epsilon_pos)
{
    ...
    const auto &true_genotypes_iter = true_genotypes_cache.find(genotype_cache_key);
    ...
    const auto &g_given_g_star_probabilities = calc_obs_genotype_lliks(obs_genotype, genotypes, epsilon_neg, epsilon_pos, genotypes.size());
    const auto &g_star_probabilities = calc_genotype_log_pmf(genotypes, coi, allele_frequencies, genotypes.size());
...
long double Chain::calc_estimated_genotype_marginal_llik(std::vector<int> const &obs_genotype, int coi, std::vector<double> const &allele_frequencies, double epsilon_neg, double epsilon_pos)
{
    ...
    const auto &importance_reweighted_frequencies = reweight_allele_frequencies(allele_frequencies, obs_genotype, epsilon_neg, epsilon_pos, coi);

    const auto &sample_true_genotypes = sampler.sample_genotype(coi, importance_reweighted_frequencies, importance_sampling_depth);
    const auto &importance_probabilities = calc_genotype_log_pmf(sample_true_genotypes, coi, importance_reweighted_frequencies, importance_sampling_depth);
    const auto &g_star_probabilities = calc_genotype_log_pmf(sample_true_genotypes, coi, allele_frequencies, importance_sampling_depth);
    const auto &g_given_g_star_probabilities = calc_obs_genotype_lliks(obs_genotype, sample_true_genotypes, epsilon_neg, epsilon_pos, importance_sampling_depth);
    ...
    {
        // divergence += exp(g_star_probabilities[i] - importance_probabilities[i]);
        // importance_prob += exp(importance_probabilities[i]);
        // target_prob += exp(g_given_g_star_probabilities[i] + g_star_probabilities[i]);
        // UtilFunctions::print("importance reweighted: ", exp(g_given_g_star_probabilities[i] + g_star_probabilities[i] - importance_probabilities[i]));
    ...
    // UtilFunctions::print("Divergence:", divergence / importance_sampling_depth);
...
{
    // Implement a way to deal with missing data here --> could either completely ignore missing data, or bias towards low COI
    ...
    double log_importance_sampling = log(params.importance_sampling_depth * coi * 10);
...
void Chain::initialize_likelihood()
{
    for (size_t j = 0; j < genotyping_data.num_loci; j++)
    {
        ...
        for (size_t i = 0; i < genotyping_data.num_samples; i++)
        {
            ...
            double marginal_llik = calc_genotype_marginal_llik(observed_alleles, m[i], p[j], eps_neg[i], eps_pos[i]);
...
Chain::Chain(GenotypingData genotyping_data, Lookup lookup, Parameters params) : genotyping_data(genotyping_data),
                                                                                 lookup(lookup),
                                                                                 params(params),
                                                                                 sampler(params.importance_sampling_depth * params.max_coi, genotyping_data.num_alleles)
{
