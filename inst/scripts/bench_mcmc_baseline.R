...
for (preset in presets) {
  ...
  for (rep in seq_len(reps)) {
    message("Running ", preset, " (rep ", rep, "/", reps, ") ...")
  ...
  message("Preset ", preset, ": mean=", round(mean(elapsed), 3), " s, sd=", round(stats::sd(elapsed), 3), " s")
  ...
  all_rows[[length(all_rows) + 1L]] <- bench_summary_rows(
    preset, burnin, samples, seed, reps, elapsed, git_sha
  )
  all_rows[[length(all_rows) + 1L]] <- bench_profiler_rows(preset, last_stats)
}
...
if (mode == "save") {
  ...
  comp <- bench_compare(current, baseline, threshold_pct = threshold, presets = presets)
